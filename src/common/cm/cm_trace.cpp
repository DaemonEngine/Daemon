...
void CM_TraceThroughLeafShader( traceWork_t *tw, const cLeaf_t *leaf )
{
	// trace line against all brushes in the leaf
	const int *firstBrushNum = leaf->firstLeafBrush;
	const int *endBrushNum = firstBrushNum + leaf->numLeafBrushes;
...
static void CM_TraceThroughTreeShader( traceWork_t *tw, int num, float p1f, float p2f, const vec3_t p1, const vec3_t p2 )
{
	cNode_t  *node;
	cplane_t *plane;
	float    t1, t2, offset;
	...
	if ( num < 0 )
	{
		CM_TraceThroughLeafShader( tw, &cm.leafs[ -1 - num ] );
		return;
	}
	...
	if ( t1 >= offset + 1 && t2 >= offset + 1 )
	{
		CM_TraceThroughTreeShader( tw, node->children[ 0 ], p1f, p2f, p1, p2 );
		return;
	}

	if ( t1 < -offset - 1 && t2 < -offset - 1 )
	{
		CM_TraceThroughTreeShader( tw, node->children[ 1 ], p1f, p2f, p1, p2 );
		return;
	}
	...
	CM_TraceThroughTreeShader( tw, node->children[ side ], p1f, midf, p1, mid );
	...
	CM_TraceThroughTreeShader( tw, node->children[ side ^ 1 ], midf, p2f, mid, p2 );
}
...
std::string CM_TraceShader( trace_t *results, const vec3_t start, const vec3_t end )
{
...

	{
		...
		//
		{
			CM_TraceThroughTreeShader( &tw, 0, 0, 1, tw.start, tw.end );
		}
	}
	...
	*results = tw.trace;
	...
	if ( tw.trace.fraction < 1.0f )
		shaderName = cm.shaders[ tw.trace.shadernum ].shader;
	return shaderName;
}
